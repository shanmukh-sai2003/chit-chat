...
function MessageItem(props) {
    ...
    return (
        <div className={`flex ${ isSender && 'flex-row-reverse'} `}>
            <div className={`text-sm ${ isSender && "text-right" } ${ isSender ? "bg-blue-500" : "bg-slate-700" } w-fit p-2 m-1 rounded-lg`}>
                { chat?.isGroupChat && !isSender && <p className="font-bold text-green-500">{senderName}</p>}
                <p className="text-white text-md">{ content }</p>
                <p className="text-slate-300 text-xs">{ moment(sentAt).format("LT") }</p>
            </div>
        </div>
    );
}
